...
// console.debug(props.parentCategories);
...
<template>
    <AdminLayout>
        <div class="max-w-xl mx-auto bg-slate-900 p-6 rounded-xl border border-slate-800 shadow-lg">
            <h1 class="text-xl font-bold text-white mb-6">Створення категорії</h1>
            ...
            <form @submit.prevent="submit" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm mb-1 text-slate-300">Батьківська категорія (необов'язково)</label>
                    <select
                        v-model="form.parent_id"
                        class="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white focus:outline-none focus:border-amber-500"
                    >
                        <option :value="null">Без батьківської (Головна категорія)</option>
                        <option v-for="parent in props.parentCategories" :key="parent.id" :value="parent.id">
                            {{ parent.name }}
                        </option>
                    </select>
