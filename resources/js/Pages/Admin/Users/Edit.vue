...
<AdminLayout>
    <div class="max-w-2xl mx-auto p-6 bg-slate-900 text-white mt-10 rounded-xl shadow">
        <h1 class="text-xl font-bold mb-6">Редагування користувача #{{ user.id }}</h1>
        ...
        <form @submit.prevent="submit" class="space-y-4">
            ...
            <div>
                <label class="block text-sm mb-1 text-slate-300">Новий пароль (залиш порожнім, якщо не змінюєш)</label>
                <input v-model="form.password" type="password" class="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white" placeholder="••••••••" />
            </div>

            <div>
                <label class="block text-sm mb-1 text-slate-300">Роль користувача</label>
