...
<template>
    <AdminLayout>
        <div class="max-w-xl mx-auto bg-slate-900 p-6 rounded-xl border border-slate-800 shadow-lg">
            <h1 class="text-xl font-bold text-white mb-6">Створення нового бренду</h1>
            ...
            <form @submit.prevent="submit" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm mb-1 text-slate-300">Опис (необов'язково)</label>
                    <textarea
                        v-model="form.description"
                        rows="4"
                        class="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white focus:outline-none focus:border-amber-500"
                        placeholder="Коротка інформація про бренд..."
                    ></textarea>
